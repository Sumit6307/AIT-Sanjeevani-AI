...
const NAV_ITEMS = [
  { href: "/dashboard/doctor", key: "nav.dashboard", icon: "📊" },
  { href: "/dashboard/doctor/appointments", key: "nav.appointments", icon: "📅" },
  { href: "/dashboard/doctor/prescriptions", key: "doctor.rxCreator.title", icon: "💊" },
  { href: "/dashboard/doctor/history", key: "doctor.history", icon: "💬" },
  { href: "/dashboard/doctor/settings", key: "nav.settings", icon: "⚙️" },
];
  ...
  return (
    <div className="grid min-h-[calc(100svh-52px)] grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <aside className="hidden border-r-2 border-border bg-card p-6 lg:sticky lg:top-0 lg:block lg:h-[calc(100svh-52px)]">
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs font-bold text-muted-foreground uppercase tracking-wide">
              {t("auth.roleSelection") === "Select your workspace role" ? "Doctor Portal" : "डॉक्टर पोर्टल"}
            </div>
          </div>
        </div>
        ...
        <div className={`mt-8 rounded-none border-2 border-border p-4 shadow-[4px_4px_0px_0px_var(--shadow)] ${isLive ? "bg-red-light" : "bg-blue-light"}`}>
          <div className="flex items-center gap-2 text-sm font-bold text-black">
            <span className={`size-3 rounded-full border border-border ${isLive ? "bg-red animate-pulse" : "bg-emerald"}`} />
            {isLive 
              ? (t("auth.roleSelection") === "Select your workspace role" ? "Live Session" : "लाइव सत्र") 
              : (t("auth.roleSelection") === "Select your workspace role" ? "Available" : "उपलब्ध")}
          </div>
          <div className="mt-1 text-xs font-medium text-black">
            {isLive 
              ? (t("auth.roleSelection") === "Select your workspace role" ? "You are in a call." : "आप कॉल पर हैं।") 
              : (t("auth.roleSelection") === "Select your workspace role" ? "Waiting for patients..." : "मरीजों की प्रतीक्षा की जा रही है...")}
          </div>
        </div>
      </aside>
      ...
      <main className="w-full overflow-x-hidden bg-background">
        <div className="border-b-2 border-border bg-card px-4 py-3 lg:hidden">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className={`rounded-none border-2 border-border px-2 py-1 text-[10px] font-bold shadow-[2px_2px_0px_0px_var(--shadow)] ${isLive ? "bg-red-light text-black" : "bg-emerald-light text-black"}`}>
              {isLive 
                ? (t("auth.roleSelection") === "Select your workspace role" ? "LIVE" : "लाइव") 
                : (t("auth.roleSelection") === "Select your workspace role" ? "AVAILABLE" : "उपलब्ध")}
            </span>
          </div>
        ...
        <div className="hidden items-center justify-between border-b-2 border-border bg-card px-6 py-4 lg:flex">
          <div className="text-sm font-bold text-muted-foreground uppercase tracking-wide">
            {t("auth.roleSelection") === "Select your workspace role" ? "Doctor Workspace" : "डॉक्टर कार्यक्षेत्र"}
          </div>
          <div className="flex items-center gap-3">
            <div className="flex size-8 items-center justify-center rounded-none border-2 border-border bg-primary text-xs font-bold text-primary-foreground">
