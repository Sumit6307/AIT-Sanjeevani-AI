...
import { z } from "zod";
import { hashPassword } from "@my-better-t-app/auth";
...
mvpRoute.post("/dev/seed-doctor", async (c) => {
  ...
  // Ensure Account record with credential provider exists
...
// In-memory cache for call participant network statuses
const callNetworkRegistry = new Map<string, Record<string, CallNetworkStatus>>();

// Cleanup stale call network data periodically (every 10 minutes)
setInterval(() => {
  const now = Date.now();
  for (const [appointmentId, participants] of callNetworkRegistry.entries()) {
    const active = Object.entries(participants).filter(([_, status]) => now - status.updatedAt < 60000);
    if (active.length === 0) {
      callNetworkRegistry.delete(appointmentId);
    } else {
      callNetworkRegistry.set(appointmentId, Object.fromEntries(active));
    }
  }
}, 600000);

// Update caller's network stats in the call registry
mvpRoute.post("/appointments/:appointmentId/network", async (c) => {
  const actor = await getActor(c);
  if ("error" in actor) return actor.error;
  
  const appointmentId = c.req.param("appointmentId");
  const { latency, mode, packetLoss, jitter } = await c.req.json();
  ...
  if (!appointment) return c.json({ error: "Appointment not found" }, 404);
  const resolvedId = appointment.id;
  
  const current = callNetworkRegistry.get(resolvedId) || {};
  current[actor.user.id] = {
    role: actor.user.role,
    latency: latency !== undefined ? latency : null,
    mode: mode || "HD_VIDEO",
    packetLoss: packetLoss !== undefined ? packetLoss : 0,
    jitter: jitter !== undefined ? jitter : 0,
    updatedAt: Date.now(),
  };
  callNetworkRegistry.set(resolvedId, current);
  
  return c.json({ success: true });
});

// Fetch other call participants' network stats from the call registry
mvpRoute.get("/appointments/:appointmentId/network", async (c) => {
  const actor = await getActor(c);
  if ("error" in actor) return actor.error;
  ...
  const participants = callNetworkRegistry.get(resolvedId) || {};
  
  const now = Date.now();
  const activeParticipants: Record<string, CallNetworkStatus> = {};
  for (const [uid, status] of Object.entries(participants)) {
    if (uid !== actor.user.id && now - status.updatedAt < 15000) {
      activeParticipants[uid] = status;
    }
  }
  
  return c.json({ participants: activeParticipants });
});

// Generate AI Medical Consultation Summary from Speech-to-Text transcript
mvpRoute.post("/appointments/:appointmentId/summary/generate", async (c) => {
  const actor = await getActor(c, ["DOCTOR"]);
  if ("error" in actor) return actor.error;
  
  const appointmentId = c.req.param("appointmentId");
  const { transcript, language } = await c.req.json();
  ...
  const systemPromptArray = [
    "You are an AI Clinical Scribe and Medical Summarizer.",
    "You will analyze the transcript of a telemedicine consultation between a doctor and a patient.",
    "Your task is to extract clinical information and construct a structured JSON report.",
    "Remove any conversational filler, chit-chat, and focus only on medically relevant facts.",
    "Use formal, professional medical terminology while keeping instructions easy for a patient to understand.",
    "Generate a structured JSON output satisfying this schema exactly:",
    "{",
    '  "patientInfo": {',
    '    "name": "string",',
    '    "age": number,',
    ...
    '    "warningSigns": ["string"]',
    '  },',
    '  "summaryText": "Concise summary (150-250 words) of the consultation",',
    '  "actionItems": {',
    '    "patient": ["string"],',
    '    "doctor": ["string"]',
    '  }',
  ...
  if (language === "hi") {
    systemPromptArray.push(
      "CRITICAL: The patient's preferred language is Hindi. You MUST write all clinical narrative keys in Hindi using clean Devanagari script. This includes: chiefComplaint, symptomsDiscussed, observations, possibleDiagnosis, notes, medicinesTable instructions, recommendedTests, diet, exercise, hydration, sleep, precautions, nextAppointment, monitoring, warningSigns, summaryText, patient action items, and doctor action items. Generic medicine names (e.g. Paracetamol, ORS) should remain in standard English/Latin characters for clarity."
    );
  } else {
    systemPromptArray.push(
      "Generate the medical report in English."
    );
  }

  const systemPrompt = systemPromptArray.join("\n");

  const userPrompt = [
    `Patient Name: ${appointment.patient.user.name}`,
    `Patient Age: ${appointment.patient.age || "Unknown"}`,
    `Patient Gender: ${appointment.patient.gender || "Unknown"}`,
    `Date: ${appointment.scheduledAt.toISOString()}`,
    `Transcript:`,
    JSON.stringify(transcript, null, 2),
  ].join("\n");

  const rawResult = await requestGeminiJson({
    model,
    systemPrompt,
    userPrompt,
    temperature: 0.2,
  ...
  const summary = await prisma.consultationSummary.upsert({
    ...
    create: {
      ...
      summaryText: (rawResult as any).summaryText || "Consultation summary",
      structuredData: rawResult as any,
      isFinalized: false,
    },
    update: {
      transcript: transcript || [],
...
// Finalize and approve the AI Consultation Summary (Doctor edit submission)
mvpRoute.patch("/appointments/:appointmentId/summary/finalize", async (c) => {
  const actor = await getActor(c, ["DOCTOR"]);
  if ("error" in actor) return actor.error;
  
  const appointmentId = c.req.param("appointmentId");
  const { summaryText, structuredData } = await c.req.json();
  ...
  const summary = await prisma.consultationSummary.update({
    where: { appointmentId: resolvedAppointmentId },
    data: {
      summaryText,
      structuredData,
  ...
  await createAuditLog(actor.user.id, "CONSULTATION_SUMMARY_FINALIZED", "Appointment", resolvedAppointmentId);

  return c.json({ summary });
});

// Get Consultation Summary (both Patient & Doctor & Admin access)
mvpRoute.get("/appointments/:appointmentId/summary", async (c) => {
  const actor = await getActor(c);
  if ("error" in actor) return actor.error;
  ...
  const isDoctorOwner = summary.appointment.doctor.userId === actor.user.id;
  const isPatientOwner = summary.appointment.patient.userId === actor.user.id;
  const isAdmin = actor.user.role === "ADMIN";

  if (!isDoctorOwner && !isPatientOwner && !isAdmin) {
    return c.json({ error: "Unauthorized access to summary" }, 403);
  }
